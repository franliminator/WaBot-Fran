    ...
    try {
      ...
      let _prefix = plugin.customPrefix ? plugin.customPrefix : conn.prefix ? conn.prefix : global.prefix
        ...
        if (!isPrems && global.DATABASE._data.users[m.sender].limit < 1 && plugin.limit) {
          this.reply(m.chat, `No tienes mas tiradas, por favor compra mas *${usedPrefix}comprar*`, m)
          continue
        }
        try {
          await plugin(m, {
          ...
          if (m.limit == true) this.reply(m.chat, 'Se aplica un límite', m)
        }
  			break
  		}
  	}
...
global.dfail = (type, m, conn) => {
  let msg = {
    owner: 'Este comando solo puede ser usado por BOT Owner !',
    mods: 'Este comando solo puede ser usado por BOT Moderador !',
    premium: 'Este comando es solo para Miembro Premium!',
    group: 'Este comando solo puede ser usado en grupos!',
    private: 'Este comando solo puede ser usado en chat privado!',
    admin: 'Estecomando solo puede ser usado por admins!',
    botAdmin: 'Dale admin al bot para poder usar este comando!'
  }[type]
