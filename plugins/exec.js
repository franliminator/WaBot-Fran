...
let handler  = async (m, { conn, usedPrefix, command, text, noPrefix, args, isOwner }) => {
  ...
  conn.clearAuthInfo = conn.groupLeave = () => {
    throw '_Seras feliz si te gusta?_'
  }
  let i = 20
  _require = m.fromMe ? require : mod => {
    ...
    if (banned.includes(mod)) return '_Qué es lo que quieres hacer?_'
    return require(mod)
  }
  try {
    ...
    }, m, handler, _require, m.fromMe ? conn : { ...conn, deleteChat: conn.clearAuthInfo }, CustomArray, {...process, exit: function exit() { return ':P' }}, args, Object.freeze({ ...global, global: 'Qué quieres aquí -_-'}))
  } catch (e) {
